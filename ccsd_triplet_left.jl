...
Ln(n) = ∑(real_tensor("c", 1:2n...) * Tdn(n), 1:2n)


function A_transpose_transform(H, X, T, L, n)
    # <HF| L [bch(H, X, n),T] | HF >

    Ω = L * commutator(bch(H, X, n), T)
...
H = F + Φ
#ρ_ai_1 = A_transpose_transform(H, Xn(2), Tn(1), 1//2*Ln(1), 2)
ρ_ai_2 = A_transpose_transform(H, Xn(2), Tn(1), 1//2*Ln(2), 2)
#ρ_aibj_2 = A_transpose_transform(H, Xn(2), 1//2*Tn(2), 1//2*Ln(2), 1)

trans = translate(VirtualOrbital => 1:2:10, OccupiedOrbital => 2:2:10)
#println("\n ρ_ai (singles):\n", (ρ_ai_1, trans))
println("\n ρ_ai (non-symmetrized doubles):\n", (ρ_ai_2, trans))
#println("\n ρ_aibj (non-symmetrized doubles):\n", (ρ_aibj_2, trans))
